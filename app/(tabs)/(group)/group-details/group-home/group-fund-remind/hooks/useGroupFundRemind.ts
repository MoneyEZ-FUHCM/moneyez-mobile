...
export default function useGroupRemind() {
  ...
  const [fundRaisingRemind, { isLoading: isRemindLoading }] =
    useFundRaisingRemindMutation();

  const { data: financialGoalData, isLoading: isGoalLoading } =
  ...
  const remainDays = deadlineDate
    ? moment(deadlineDate).diff(moment(), "days")
    : 0;
  ...
  useEffect(() => {
    if (groupDetail?.groupMembers) {
      const mappedMembers = groupDetail.groupMembers
        ...
        .map((member) => {
          const target =
            isGoalActive && hasFinancialGoal && groupGoal > 0
          ...
          const hasFundedEnough =
            hasFinancialGoal &&
            target > 0 &&
            member.totalContribution >= target;
  ...
  }, [groupDetail, isGoalActive, groupGoal, hasFinancialGoal, userInfo]);
  ...
  const handleToggleMember = useCallback((id: string) => {
    setMembers((prev) =>
      prev.map((m) =>
        m.id === id && !m.disabled ? { ...m, checked: !m.checked } : m,
      ),
    );
  }, []);
  ...
  const handleToggleAll = useCallback(() => {
    ...
    setMembers((prev) =>
      prev.map((m) =>
        m.disabled ? m : { ...m, checked: !allAvailableChecked },
      ),
    );
  }, [members]);

  const handleCreateRemind = useCallback(
    async (values: { amount: string; description: string }) => {
      if (!groupDetail?.id) {
        ToastAndroid.show(
          TEXT_TRANSLATE_GROUP_REMIND.MESSAGE_ERROR.CREATE_FAILED,
          ToastAndroid.SHORT,
        );
        return;
      }
      ...
      try {
        ...
        if (checkedMembers.length === 0) {
          ToastAndroid.show(
            TEXT_TRANSLATE_GROUP_REMIND.MESSAGE_VALIDATE.SELECT_MEMBER_MIN,
            ToastAndroid.SHORT,
          );
          return;
        }
