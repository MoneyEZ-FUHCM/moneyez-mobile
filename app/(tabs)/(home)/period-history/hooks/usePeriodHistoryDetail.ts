...
const usePeriodHistoryDetail = () => {
  ...
  const { data: transactionsData, error, isLoading, refetch } = useGetTransactionByModelQuery(
  ...
  useEffect(() => {
    if (transactionsData) {
      if (transactionsData.totalCount) setTotalCount(transactionsData.totalCount);
  ...
  const filteredTransactions = useMemo(() => {
    return transactions.filter(transaction => {
      ...
      const matchesSubcategory = selectedSubcategory ? transaction.subcategoryId === selectedSubcategory : true;
